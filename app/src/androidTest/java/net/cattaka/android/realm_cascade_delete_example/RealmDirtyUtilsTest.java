...
import io.realm.RealmDirtyUtils;
 ...
 */
public class RealmDirtyUtilsTest {
    private Realm mRealm;
    private RealmDirtyUtils mSup;

    @Before
    public void setUp() throws Exception {
        RealmConfiguration config = new RealmConfiguration.Builder()
                .name("RealmDirtyUtilsTest")
                .schemaVersion(1)
                .build();
        ...
        mSup = new RealmDirtyUtils(config);
    }
    ...
    public void test_cleanUp() {
        ...
        {   // Nothing are deleted
            Map<Class<? extends RealmModel>, Integer> results = mSup.cleanUp(mRealm, false);
    ...
    @Test
    public void test_deleteFromRealmAndCleanUp() {
        List<Page> pages = Arrays.asList(
                new Page("aaa"),
                new Page("aab"),
                new Page("aba"),    // not used
        ...
        {   // Delete some parent objects
            List<Book> realmBooks = mRealm.where(Book.class).findAll().sort("id");
            mRealm.beginTransaction();
            mSup.deleteFromRealmAndCleanUp(realmBooks.get(0).getChapters().get(0).getPages().remove(2));
            mSup.deleteFromRealmAndCleanUp(realmBooks.get(1).getChapters().get(1));
            mSup.deleteFromRealmAndCleanUp(realmBooks.get(1).getIndexItems().get(1));
            mSup.deleteFromRealmAndCleanUp(realmBooks.get(1).getIndexItems().get(0));
            mSup.deleteFromRealmAndCleanUp(realmBooks.get(2));
            mRealm.commitTransaction();
        }
